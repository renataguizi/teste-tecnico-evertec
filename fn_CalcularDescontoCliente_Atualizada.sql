...
BEGIN
    ...
    IF ISNULL(@QtdCategorias, 0) >= 3
		...
		-- VERIFICA QUANTOS ITENS DIFERENTES CADA CLIENTE COMPROU NO ULTIMO ANO
    SELECT 
    ...
    WHERE V.ClienteID = @ClienteID
	  ...
	  -- DESCONHEÇO SOBRE OS OPERADORES += E -=
...
-- ESSA PARTE FICOU UM POUCO CONFUSA, ELE DEVERIA FAZER ALGUM CÁLCULO? OU SETAR TODOS OS CANCELAMENTOS RECENTES COM DESCONTO DE 0,50?    IF ISNULL(@QtdCancelamentosRecentes, 0) > 0 
		SET @Desconto = 0.50;
